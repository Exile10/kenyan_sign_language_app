...
import com.jerry.ksl.gesturerecognizer.databinding.ItemGestureRecognizerResultBinding
...
class GestureRecognizerResultsAdapter :
    ...
    fun updateResults(categories: GestureRecognizerResult?) {
        adapterCategories = MutableList(adapterSize) { null }
        if (categories != null) {
            val gestures = categories.gestures()
            if (!gestures.isEmpty() && !gestures[0].isEmpty()) {
                val sortedCategories = gestures[0].sortedByDescending { it.score() }
